...
func (o *Orchestrator) tickIdle(group config.GroupConfig, profile *profiles.Profile, mainVol string, gs *state.GroupState) error {
	if o.shouldCheckUpdate(group, gs) {
		if err := o.checkAndRecordUpdate(group, profile, mainVol, gs); err != nil {
			return err
		}
	} else {
	...
	if gs.TargetBuildID != "" {
		return o.proceedPendingUpdate(group, gs)
	}
...
func (o *Orchestrator) checkAndRecordUpdate(group config.GroupConfig, profile *profiles.Profile, mainVol string, gs *state.GroupState) error {
	...
	o.log.Step(ui.StatusOK, "update available (buildid %s)", info.Remote)
	if gs.UpdateDetectedAt.IsZero() || gs.TargetBuildID != info.Remote {
		gs.UpdateDetectedAt = time.Now().UTC()
		gs.TargetBuildID = info.Remote
...
func (o *Orchestrator) proceedPendingUpdate(group config.GroupConfig, gs *state.GroupState) error {
...
func (o *Orchestrator) finishChildren(groupName string, gs *state.GroupState) error {
	gs.Phase = state.PhaseIdle
	gs.ClearUpdatePending()
	o.log.Step(ui.StatusOK, "all children synced — back to idle")
